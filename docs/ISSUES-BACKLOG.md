...
| Data | Issue |
|------|-------|
| 2026-09-21 | **GA4 a zero utilizadores desde que existe pré-render** — `loadGoogleAnalytics()` injectava `<script id="ga-gtag">` em runtime, o `scripts/prerender.js` gravava esse script no HTML de cada rota, e na visita real o guard "script já existe" fazia early-return: o gtag.js carregava **sem** `gtag('config')` e o site não enviava um único pageview. Snippet passou a estático no `index.html`; `analytics.ts` ficou só com `disableGoogleAnalytics()`; o pré-render bloqueia pedidos ao GA para não medir os builds |
